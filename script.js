...
let siteData = normalizeSiteData(DEFAULT_SITE_DATA);
...
async function loadSiteData() {
  try {
    ...
    if (!response.ok) throw new Error(`Não foi possível carregar ${CONFIG_URL}`);
    const json = await response.json();
    siteData = normalizeSiteData(json);
  } catch (error) {
    console.warn("Usando dados padrão porque o JSON estático não foi carregado.", error);
    siteData = normalizeSiteData(DEFAULT_SITE_DATA);
  }
}
...
function renderTexts() {
  ...
  Object.entries(t).forEach(([key, value]) => {
    ...
    if (el && key !== "heroImage" && key !== "heroImageAlt" && typeof value !== "object") el.textContent = value;
  });
  $("#heroImage").src = t.heroImage;
  $("#heroImage").alt = t.heroImageAlt;
  $("#heroImage").style.cssText = imageFitStyle(t.heroImageFit);
  $("#albumClose").ariaLabel = t.galleryCloseLabel;
  $("#albumPrevious").ariaLabel = t.galleryPreviousLabel;
  $("#albumNext").ariaLabel = t.galleryNextLabel;
}

function renderContacts() {
  $("#contactList").innerHTML = siteData.contacts.map(c => `<a class="contact-item" href="${escapeHtml(c.link)}" target="_blank" rel="noopener"><span>${escapeHtml(c.type)}</span><strong>${escapeHtml(c.value)}</strong></a>`).join("");
}

function renderPackages() {
  $("#packageGrid").innerHTML = siteData.packages.map(p => `<article class="package-card"><h3>${escapeHtml(p.name)}</h3><p class="price">${escapeHtml(p.price)}</p><p>${escapeHtml(p.description)}</p><ul>${(p.items || []).map(i => `<li>${escapeHtml(i)}</li>`).join("")}</ul><a class="btn small" href="#contato">${escapeHtml(siteData.texts.packageButton)}</a></article>`).join("");
}

function renderTestimonials() {
  $("#testimonialGrid").innerHTML = siteData.testimonials.map(i => `<article class="testimonial-card"><p>“${escapeHtml(i.text)}”</p><strong>${escapeHtml(i.name)}</strong></article>`).join("");
}

function renderPortfolioFilters() {
  const categories = ["Todos", ...new Set(siteData.portfolio.map(i => i.category).filter(Boolean))];
  if (!categories.includes(currentCategory)) currentCategory = "Todos";
...
function openAlbum(index) {
  currentAlbum = siteData.portfolio[index];
  currentImageIndex = 0;
  renderAlbum();
...
function renderAlbum() {
  ...
  $("#albumImage").alt = `${currentAlbum.title} - ${currentImageIndex + 1}`;
  $("#albumTitle").textContent = currentAlbum.title;
  $("#albumMeta").textContent = `${currentAlbum.category} • ${siteData.texts.galleryCounterLabel} ${currentImageIndex + 1} de ${images.length}`;
...
function setupAlbum() {
  ...
  $("#albumPrevious").addEventListener("click", () => { currentImageIndex--; renderAlbum(); });
  $("#albumNext").addEventListener("click", () => { currentImageIndex++; renderAlbum(); });
  document.addEventListener("keydown", e => {
    if (!$("#albumLightbox").classList.contains("show")) return;
    if (e.key === "Escape") closeAlbum();
    if (e.key === "ArrowLeft") { currentImageIndex--; renderAlbum(); }
    if (e.key === "ArrowRight") { currentImageIndex++; renderAlbum(); }
  });
}
...
function setupScrollReveal() {
  ...
  $$(".section-reveal").forEach(s => observer.observe(s));
}
